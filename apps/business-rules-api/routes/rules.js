...
router.post('/validate-with-rule', async (req, res) => {
  try {
    const { rule_id, data, tipo_operacion } = req.body;

    if (!rule_id || !data || !tipo_operacion) {
      return res.status(400).json({
        error: 'rule_id, data y tipo_operacion son requeridos'
      });
    }

    // Obtener la regla de la base de datos
    const ruleQuery = 'SELECT * FROM reglanegocio WHERE id = $1';
    ...
    // Generar id_display dinámicamente (formato: REG-YYYYMMDD-ID)
    ...
    console.log(`Validando ${data.length} registros con regla ${id_display}`);

    // Validar datos con Gemini
    const validationResult = await geminiService.validateMappingDataWithRule(
      rule,
      data,
      tipo_operacion
    );
...
router.get('/relevant', async (req, res) => {
  try {
    const { tipo_operacion, user_id } = req.query;

    if (!tipo_operacion) {
      return res.status(400).json({
        error: 'tipo_operacion es requerido'
      });
    }
    ...
    let query = `
      SELECT
        id, resumen, status,
        regla_estandarizada, fecha_creacion
      FROM reglanegocio
      WHERE status = 'Activa'
    `;
    ...
    console.log(`Encontradas ${result.rows.length} reglas activas para usuario ${user_id}`);

    // Agregar id_display dinámicamente y filtrar por tipo de operación
    const rulesWithDisplay = result.rows.map(rule => {
      const fechaCreacion = new Date(rule.fecha_creacion);
      const year = fechaCreacion.getFullYear();
      const month = String(fechaCreacion.getMonth() + 1).padStart(2, '0');
      const day = String(fechaCreacion.getDate()).padStart(2, '0');
      ...
      return {
        ...rule,
        id_regla: rule.id, // Mantener compatibilidad con frontend
        id_display: id_display
      };
    });

    const relevantRules = rulesWithDisplay.filter(rule => {
      // Si no tiene regla_estandarizada, incluir de todas formas
      if (!rule.regla_estandarizada) {
        return true;
      }

      // Buscar palabras clave en el JSON de la regla Y en el resumen
      const ruleText = JSON.stringify(rule.regla_estandarizada).toLowerCase();
      const resumenText = (rule.resumen || '').toLowerCase();
      const fullText = ruleText + ' ' + resumenText;

      if (tipo_operacion === 'proveedores') {
        return fullText.includes('proveedor') ||
               fullText.includes('pago') ||
               fullText.includes('transferencia') ||
               fullText.includes('spei') ||
               fullText.includes('cuenta') ||
               fullText.includes('beneficiario') ||
               fullText.includes('monto') ||
               fullText.includes('importe') ||
               fullText.includes('transacción') ||
               fullText.includes('transaccion');
      } else if (tipo_operacion === 'nomina') {
        return fullText.includes('nomina') ||
               fullText.includes('nómina') ||
               fullText.includes('empleado') ||
               fullText.includes('salario') ||
               fullText.includes('clabe') ||
               fullText.includes('rfc') ||
               fullText.includes('curp');
      }

      // Si no es ni proveedores ni nómina, incluir todas las reglas
      return true;
    });

    console.log(`${relevantRules.length} reglas son relevantes para ${tipo_operacion}`);

    res.json({ rules: relevantRules });
