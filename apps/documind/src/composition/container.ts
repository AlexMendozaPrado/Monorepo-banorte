...
import { OpenAISentimentExtractor } from "@/infrastructure/llm/OpenAISentimentExtractor";
...
export function makeAnalyzePdfUseCase(): AnalyzePdfUseCase {
  ...
  const sentimentExtractor = new OpenAISentimentExtractor(openai, model);

  return new AnalyzePdfUseCase(textExtractor, keywordExtractor, sentimentExtractor);
}
